...
- 🆕 New `theme-layout: foo` front matter key to explicitly target a theme layout
...
* `layout: :theme/foo` is deprecated, use `layout: foo` (resolves local first, theme fallback) or `theme-layout: foo` (explicit theme).
* If you have custom layouts in `templates/layouts/{theme-name}/`, move them to `templates/layouts/`.
* Roq plugins and themes must use the same version as roq.
* `page.date` is now `null` for normal pages without a date. If your layouts use `page.date` on non-collection pages, wrap with `{#if page.date}{page.date.format('yyyy, MMM dd')}{/if}`.

If you have custom plugins:

* Consuming `RoqFrontMatterRawTemplateBuildItem` or `RoqFrontMatterScanProcessor` must be updated — these are replaced by the new step-based build items.
* Build item packages moved: `deployment.scan.\*` → `deployment.items.scan.*`, `deployment.data.*` → `deployment.items.data.*`, `deployment.publish.*` → `deployment.items.publish.*`. New packages: `items.assemble.*`, `items.record.*`.
* Constants like `PAGINATE_KEY` and `LINK_KEY` moved to `RoqFrontMatterKeys` (runtime module).
