...
`roq-base/default`'s `<html>` and `<body>` tags each accept an optional CSS class from FrontMatter, so a layout extending it (`layout: roq-base/default`) doesn't need to redefine the whole document just to add its own classes:

[source,yaml]
----
html-class: bg-slate-100 dark:bg-slate-900
body-class: font-sans
----

These keys can be set on a page, or on a layout (e.g. a theme's own `default.html`) to apply a default class to every page using it; a page's own value replaces the layout's, like any other FrontMatter key. When neither is set, no `class` attribute is rendered at all.

TIP: The camelCase `htmlClass`/`bodyClass` keys are deprecated. They still work: each template's FrontMatter is normalized before the layout chain is merged, so an old key is read as its hyphenated counterpart and a build warning points at the file to fix. If a file sets both, the hyphenated key wins and the deprecated one is ignored.
...
| All
| `:dir`
| The directory portion of the page’s file path, slugified. For index files (directory pages), this is the grandparent directory (since the slug already captures the directory name). Empty for files at the content root. Use :dir[N] to skip the first N path segments — for example, :dir[2] drops the first two folders, giving a link of `my-post/my-file` for content in `/content/posts/2025/my-post/my-file.md`.
...
The `roq` command installed by JBang is a small launcher for the `roq@quarkiverse/quarkus-roq` alias. Each time it runs, JBang resolves this alias against the online catalog (`jbang-catalog.json`) so that you always get the latest Roq CLI. The catalog is cached for 12 hours: after that, running `roq` without network access fails with:

[source]
----
[jbang] [ERROR] Could not download https://github.com/quarkiverse/quarkus-roq/blob/HEAD/jbang-catalog.json
----

To work offline, run the Roq CLI through JBang in offline mode. JBang then uses the cached catalog and dependencies instead of contacting the network:

[source,shell]
----
jbang --offline run roq@quarkiverse/quarkus-roq generate --offline
jbang --offline run roq@quarkiverse/quarkus-roq start --offline
----

* The first `--offline` (before `run`) is for JBang: use the cached catalog and dependencies, never go online.
* The second `--offline` (after the Roq command) is passed to Maven or Gradle so that the build itself does not try to download anything.

NOTE: This requires having run `roq` online at least once (to cache the CLI), and having built the project online at least once (to cache its dependencies in the local Maven or Gradle repository).

TIP: If you often work offline, define a shell alias, for example `alias roq-offline='jbang --offline run roq@quarkiverse/quarkus-roq'`, then use `roq-offline generate --offline`.
