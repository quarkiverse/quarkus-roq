...
== Setting the Root Path for your site (sub-path)

When the entire Roq site is under a sub-path, configure `quarkus.http.root-path` in you Quarkus configuration.

Environment variable: `SITE_ROOT_PATH`

TIP: For GitHub Pages, this is already handled by the Roq GitHub Action, no need to do anything.
