...
[WARNING]
====
Layouts, Partials and User tags must use one of the following extensions: 
`html`, `xhtml`, `htm`, `json`, `yaml`, `yml`, or `xml`.

Within templates, you can enable markup through Qute sections `{#md}` and `{#adoc}`, provided the corresponding plugins are available.
====
