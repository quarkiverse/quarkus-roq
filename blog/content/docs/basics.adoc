...
Content templates can be written in `html`, `json`, `yaml`, `yml`, or `xml` or using <<Markup languages>>. Other text formats (such as `.txt`) need the <<qute-suffix, `.qute.` suffix>> to be processed.
...
[#qute-suffix]
=== Other text files (`.qute.` suffix)

Only files with a template extension are processed and served from `content/`. By default, these are `html`, `xhtml`, `htm`, `json`, `yaml`, `yml`, `xml` and the <<Markup languages, markup>> extensions (`md`, `adoc`, ...).

To serve another text file as a template (e.g. `llms.txt`, `humans.txt`), insert `.qute` before its extension. The `.qute` part is removed from the generated file name:
...
└── humans.qute.txt // <2>
----
<1> generated as `/llms.txt`
<2> generated as `/humans.txt`

WARNING: A non-template file placed in `content/` is not a page. It is only served when it is <<page-files, attached to a page>> (next to an `index` page in a subdirectory). A file like `content/llms.txt` is *not* served. Use `content/llms.qute.txt` if it needs Qute, or put it in `public/` if it is a plain <<site-static, static file>>.

If you prefer to keep the original file names, you can add extensions to the list of template suffixes in your `config/application.properties`. You must keep the default values in the list:

[source,properties]
----
quarkus.qute.suffixes=html,xhtml,htm,json,yaml,yml,xml,qute.txt,txt
----

CAUTION: With `txt` in the list, *every* `.txt` file in `content/` is processed as a Qute template, including files attached to pages. This is why `txt` is not a default suffix. Any `{` in those files may be parsed as a Qute expression.
...
Markdown rendering is provided by https://docs.quarkiverse.io/quarkus-qute-web/dev/markdown.html[Quarkus Qute Web Markdown,window=_blank], which bundles the following commonmark-java extensions. They are all enabled by default and can be turned on or off individually with configuration properties:
...
| Tables
| `quarkus.qute.web.markdown.plugin.tables.enabled`
| GitHub Flavored Markdown tables (`\| a \| b \|`).

| Autolink
| `quarkus.qute.web.markdown.plugin.autolink.enabled`
| Turns bare URLs and email addresses into links without needing the `<...>` or `[...]()` syntax.

| Heading anchor
| `quarkus.qute.web.markdown.plugin.heading-anchor.enabled`
| Adds an `id` attribute to every heading (e.g. `## My Title` becomes `<h2 id="my-title">`), so headings can be linked to with `#my-title`.

| Alerts
| `quarkus.qute.web.markdown.plugin.alerts.enabled`
| GitHub Flavored Markdown alert blocks (`> [!NOTE]`, `> [!TIP]`, `> [!IMPORTANT]`, `> [!WARNING]`, `> [!CAUTION]`).
|===

All properties are booleans and default to `true`. For example, to disable autolinks and tables:

[source,properties]
.application.properties
----
quarkus.qute.web.markdown.plugin.autolink.enabled=false
quarkus.qute.web.markdown.plugin.tables.enabled=false
----

The alerts extension also lets you register custom alert types (the key must be uppercase, the value is the title displayed in the block):
...
NOTE: The heading anchor extension only adds the `id` attributes, it does not render a visible link icon next to headings. Add your own CSS/JS if you want clickable anchors.

TIP: See the https://docs.quarkiverse.io/quarkus-qute-web/dev/markdown.html#extension-configuration-reference[Quarkus Qute Web configuration reference,window=_blank] for the complete list of Markdown properties.
...
[source]
----
site.collections.docs.layout="page" // <1>
site.collections.docs.future=true // <2>

site.collections.posts.layout="post"
----

<1> Here, we set the layout for docs pages to `page`;
<2> Since the new collection is not a time-based collection, we need to set `future` as true to show all files.
...
image::roq-layouts.webp[Roq - Layouts]
...
Set a `robots` value in a page's (or document's) FrontMatter to control how search engines index it. When the page is rendered, the `{#seo page site /}` tag will emit a corresponding `<meta name="robots">` tag.
...
* `noindex` — exclude the page from search engine indexes (drafts, internal docs, staging content)
* `nofollow` — don't follow links from this page
* `noindex, nofollow` — combine both

The `<meta name="robots">` tag is only rendered when the `robots` key is set on the page.

Configure site-wide SEO defaults through the site index frontmatter:
...
NOTE: The `.qute` part is required for `.txt` files to be processed as templates, it is removed from the generated file name (see <<qute-suffix>>).
...
Access it with the `cdi` namespace:
...
A directory inside `data/` is automatically grouped into a single bean, with each file as a key (filename without extension):
...
The bundled files will be generated in `/static/bundle/...`,  you can see them in the target directory or the output of generation, but you don't have to reference them manually.

Instead, to include the generated bundle in your templates (the base theme and the others already includes it), specify the bundle user tag in the `html>head` tag:
