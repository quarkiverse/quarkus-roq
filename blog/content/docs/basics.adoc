...
[INFO]
====
`.md` and `.adoc` files are not parsed as templates.  
Instead, you can use markup inside templates through Qute sections `{#md}` and `{#adoc}`, provided the corresponding <<Markup languages,plugins>> are available.
====
