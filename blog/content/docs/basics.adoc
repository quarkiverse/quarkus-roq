...
It will provide templates, scripts and styles for your site. Theme layouts are automatically available: just use `layout: foo` and it resolves local first, then falls back to the theme. For advanced usage, refer to the xref:advanced.adoc#themes[*Theme section*].
...
<2> The layout to use (in this case `index` which resolves to the `index` layout, local first then theme fallback).
...
----
<1> This time we use the `post` layout (resolves local first, then theme fallback).
...
site.collections.docs.layout="post" //<2>
