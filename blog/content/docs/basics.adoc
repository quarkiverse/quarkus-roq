...
The `<meta name="robots">` tag is rendered when the `robots` key is set on the page, or as `noindex` when the page sets `no-index-all: true` (see <<no-index-all>>).

[[no-index-all]]
==== Excluding a page from all indexes (per page)

Set `no-index-all: true` in a page's (or document's) FrontMatter to keep it out of every index at once: the link:/plugin/sitemap/[sitemap], <<llmstxt,llms.txt>>, the link:/plugin/lunr-search/[search index], and any other indexation plugin. It also defaults the `robots` meta tag to `noindex`.
...
A plugin key set on the same page wins over it. The check order is: the plugin key (`sitemap`, `llmstxt`, `search`), then `no-index-all`, then the default (indexed). For example, `no-index-all: true` with `llmstxt: true` keeps the page in llms.txt only, and `robots: "index, follow"` keeps that explicit `robots` value.
...
To exclude a specific page, set `llmstxt: false` in its frontmatter. A page with `no-index-all: true` (see <<no-index-all>>) is excluded too, unless it sets `llmstxt: true`.
