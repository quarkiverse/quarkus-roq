...
* `templates/partials/post-link.html`  
  with a `DocumentPage` parameter named `post`  
  can be used as: `{#post-link post/}`  
  to display a styled card linking to another post.
