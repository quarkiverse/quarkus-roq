...
Roq already includes the Markdown plugin by default. To use Markdown markup language (based on https://github.com/commonmark/commonmark-java[commonmark-java,window=_blank]), on pages or documents, add the `.md` or `markdown` file extension and it will automatically work.

TIP: If you don't want to use Markdown at all, you may disable it by excluding the Markdown plugin from the Roq extension in your `pom.xml`.

=== Asciidoc

Asciidoc is also fully compatible with Roq using xref:plugins.adoc#plugin-asciidoc[plugins].
