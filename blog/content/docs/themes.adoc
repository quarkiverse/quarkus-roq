...
Theme layouts are automatically available: just use `layout: foo` and it resolves local first, then falls back to the theme. For explicit theme targeting, use `theme-layout: foo`.
...
// TODO Claude: add override instructions layouts and css, add more info about configuration from site index and available partials/tags/layouts
...
The _bio_ data type now support several improvements for modern, 21st century resumes:

* A `Logo` type, composed of:
** a required `label`
** a required `imageUrl`
** an optional `link` URL

-> This type is useful to render a company logo on your resume.

* a `subItems` element, which is a list of regular items

-> This allows you to nest items in items in items ... you got it. Particularly useful for consulting professionals working on several missions for the same company.

* a `collapsible` flag, to allow expanding/collapsing an item
* a `collapsed` flag, to define an item's default state when rendering your resume
* a `ruler` flag, to draw an horizontal ruler after an item content
...
[source,yaml]
----
- title: Mega-corporations
  items:
    - logo:
        link: https://abc.xyz/
        label: Alphabet Inc.
        imageUrl: logos/sample/alphabet_footer_logo.png
      header: Alphabet Inc.
      ...
      subItems:
        - logo:
            ...
            label: Google
            imageUrl: logos/sample/google.png
          header: Google
          title: Google (title)
          collapsible: false
          ruler: true
        ...
        - logo:
            ...
            label: Waymo
            imageUrl: logos/sample/waymo.webp
          header: Waymo
          title: Waymo (title)
          collapsible: true
          collapsed: false
        ...
        - logo:
            ...
            label: Verily
            imageUrl: logos/sample/verily-logo.svg
          header: Verily
          title: Verily (title)
          collapsible: true
          collapsed: true
          ...
          content: |
            *Verily*
            ...
            Life sciences
----
is initially rendered as this:

image::themes/roq-resume-bio-hierarchical.png[Roq hierarchical bio,500]
