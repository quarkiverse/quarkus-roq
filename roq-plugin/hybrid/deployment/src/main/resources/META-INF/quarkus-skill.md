...
description: "Enable hybrid mode for building Quarkus applications with Roq static content, adding runtime page caching, future page scheduling, and cache management."
categories: web
---

# Quarkus Roq Hybrid Plugin

The hybrid plugin enables building Quarkus applications with Roq static content. Pages are rendered once and cached (in memory or on disk), making subsequent requests as fast as serving static files while still supporting dynamic CDI content.
...
- `lazy` (default): rendered on first request, cached for subsequent requests. Supports `cache-ttl` for expiration.
- `startup`: pre-rendered at application startup, cached for the lifetime of the application. TTL is ignored.
...
The hybrid handler sets `Last-Modified` headers on cacheable pages and supports `304 Not Modified` via `If-Modified-Since`, following the same pattern as Vert.x StaticHandler. Pages with `cache: false` do not get these headers.
...
- `get(String key, Page page)` returns the cached entry (content + cachedAt) or null, checks per-page TTL
...
- `cacheKey(Page page)` computes the cache key for a page (output path with target extension, e.g. `pages/about.html`)
...
| `site.hybrid.cache-store` | `memory` | Cache backend: `memory` or `filesystem` |
| `site.hybrid.cache-dir` | (temp dir) | Directory for filesystem cache (filesystem store only) |
| `site.hybrid.cache-max-size` | `1000` | Max lazy-cached pages (memory store only, startup pages are not counted) |
