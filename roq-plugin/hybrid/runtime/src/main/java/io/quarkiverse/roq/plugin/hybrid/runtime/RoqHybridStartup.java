...
public class RoqHybridStartup {
    ...
    public void cacheStartupPages() {
        ...
        DefaultRoqPageRenderer renderer = Arc.container().instance(DefaultRoqPageRenderer.class).get();
        ...
        for (var entry : pages.entrySet()) {
            ...
            for (String locale : getLocales(page)) {
                try {
                    String rendered = renderer.render(page, template, locale)
                            .toCompletableFuture().join();
                    if (rendered != null) {
                        String cacheKey = cacheManager.cacheKey(page);
                        cacheManager.put(cacheKey, rendered);
