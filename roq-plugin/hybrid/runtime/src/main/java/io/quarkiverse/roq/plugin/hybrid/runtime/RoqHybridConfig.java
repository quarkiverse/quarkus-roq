...
public interface RoqHybridConfig {
     ...
     * Can be overridden per-page via frontmatter {@code cache: lazy|startup|false}.
     */
    @WithDefault("lazy")
    CacheMode cacheMode();
    ...
    /**
     * Maximum number of lazy-cached pages to keep in the memory cache.
     * When exceeded, the least recently used entries are evicted.
     * Only applies to the memory store. Startup-cached pages are not counted.
     */
    @WithDefault("1000")
    int cacheMaxSize();

    /**
     * Time-to-live for lazy-cached pages. When set, cached entries expire after this duration
     * and will be re-rendered on the next request. Startup-cached pages ignore this setting.
     * Can be overridden per-page via frontmatter {@code cache-ttl}.
