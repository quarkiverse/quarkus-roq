...
public class TemplateLink {
    ...
    private static Map<String, Supplier<String>> withBasePlaceHolders(LinkData data, Map<String, Supplier<String>> other) {
        Map<String, Supplier<String>> result = new HashMap<>(Map.ofEntries(
                ...
                Map.entry(":file", () -> slugify(removeDate(data.pageSource().baseFileName()), true, false)
                        .toLowerCase()),
                Map.entry(":File", () -> slugify(removeDate(data.pageSource().baseFileName()), true, false)))); // Case-preserving slug
        if (other != null) {
            result.putAll(other);
        }
        return result;
