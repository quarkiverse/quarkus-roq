...
public class RemoteContentSelection {
    ...
    RoqSelection produce() throws IOException {
        // Fetch content list from remote API at build time
        List<Post> posts = apiClient.getAllPosts(); // <2>

        // Download a JavaScript file from the API
        ...
        Files.writeString(publicDir.resolve("analytics.js"), analyticsScript); // <3>
        ...
        paths.addAll(posts.stream()
            .map(post -> SelectedPath.builder()
                .html("/blog/" + post.slug) // <5>
                .build())
            .toList());
...
<2> Fetch the list of posts at build time (when `mvn package quarkus:run` runs)
<3> Download a JavaScript file from the API and write it to the public/ directory
<4> Add a selection path for the downloaded file so it gets included in the static site
<5> Generate a static HTML page for each post: `/blog/hello-world/index.html`, etc.
