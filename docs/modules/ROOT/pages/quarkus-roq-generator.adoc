...
TIP: When used within the Roq Static Site Generator extension, it is already pre-configured to generate the whole site (which can be disabled with `quarkys.roq.site.generator=false`). In standalone mode, by default, only the `/` (index.html) and `static/**` will be generated. Follow the next section to configure the selection.
