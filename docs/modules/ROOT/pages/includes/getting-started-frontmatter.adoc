...
// https://excalidraw.com/#json=agpYoI4FLPUDHIuS2qWZ2,_qAVC8dqhVn8Q-dJtj84yA
...
For your site, you will have one or more kind of pages, this is what we call "layouts", located by default in `site/_layouts/`. For example:
...
You can also prepare partial templates. They are located by default in `site/_includes_/`. For example:

* `pagination.html`
* `head.html`

NOTE: if you don't already have layouts and includes in your project, copy them from the https://github.com/quarkiverse/quarkus-roq/tree/main/blog/site[Roq Blog].

=== Site index template

Your site index template is required and should be located in `site/index.html`. In your templates, you can use Qute templating engine with type-safety and all.
...
* The `site` variable allow to access site global info from any page, document, layout or include.
+
.Show attributes
[%collapsible]
====
...
* The `page` variable is available in pages, documents, layouts, and includes. It contains the info for the page it is used from.
+
.Show attributes
[%collapsible]
====
...
<1> you can use `link` to give this page a custom link.
...
By default, Roq is configured with a `posts` collection using the `site/_posts` directory. Let's create our first post:

[source,html]
._posts/2024-10-14-roq-solid.md
----
---
title: Roq bad puns
...
----
<1> This time we use the `_layouts/post.html` layout.
...
By default, all files in `site/static/` are treated as static. It is possible to configure other files to be static.
